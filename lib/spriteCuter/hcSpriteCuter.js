...
var fs = require('fs-extra'), PNG = require('pngjs').PNG;
...
module.exports = (pngFile) => {
    return new Promise((resolve, reject) => {
        ...
        // const buffer = PNG.sync.write(png, { filterType: 4 });
        // var b = fs.writeFileSync('out.png', buffer);
        const tempData = Buffer.alloc(4 * png.width * png.height);
        png.data.copy(tempData);

        if (!fs.existsSync(outDirPath)) {
            fs.mkdirSync(outDirPath);
        }

        var spritesArray = getSprites(tempData, png.height, png.width);

        // const datass = fs.readFileSync('E:\\project\\puppeteer-download\\file\\切图资源\\洞房花柱\\image\\bigwin_01595219325313\\0.png');
        // console.log('datass', datass);
        // return

        // console.log('spritesArray', spritesArray);
        for (let i = 0; i < 2; i++) {
            ...
            // const data = fs.readFileSync(outDirPath + '\\' + i + '.png');
            // const png = PNG.sync.read(data);
            // const buffer = PNG.sync.write(png, { filterType: 4 });
            // fs.writeFileSync(outDirPath + '' + i + '.png', buffer);

            PNG.bitblt(newPng, rect.lt.x, rect.lt.y, newData.width, newData.height, 0, 0);
            // var dst = fs.createWriteStream(outDirPath + '/' + i + '.png');

            // console.log('outDirPathoutDirPath', outDirPath + '\\' + i + '.png');
            // var datas = fs.readFileSync(outDirPath + '\\' + '1.png');

            // var datas = fs.readFileSync(outDirPath + '\\' + i + '.png');
            // console.log('datas', datas);
            // var png = PNG.sync.read(datas);
        ...
        //         // fs.writeFile(outDirPath + '.css', css, function (err) {
        //         //     if (err) throw err;
        //         //     console.log('It\'s saved!');
        //         //     resolve('ok')
        //         // });

        //         // spritesArray.map(async rect => {
        //         //     var newData = { data: null, height: (rect.rb.y - rect.rt.y), width: (rect.rt.x - rect.lt.x) };
        //         //     var newPng = new PNG({
        //         //         filterType: 4,
        //         //         width: newData.width,
